...
/* ===== Documentation ===== 
Name: Homework_class_teacher
Role: Controller
Description: Fees Class controls access to all homework pages and functions from the class teacher's end
...
class Homework_class_teacher extends MY_Controller {
	public function __construct() {
		...
		$this->teacher_class_id = $this->class_details->id;
		//get school id
	...
	public function homework() {
		...
		$config["base_url"] = base_url('homework_class_teacher/homework');
		...
		$this->load->view('staff/homework/class_teacher/homework', $data);
		$this->staff_footer();
	}
	...
	public function view_homework($homework_id) {
		...
		$this->restrict_homework($homework_id, $this->teacher_class_id);
		...
		$this->load->view('staff/homework/class_teacher/view_homework', $data);
        $this->staff_footer();
	}
	...
	/* ====== New Homework ====== */
	public function new_homework($error = array('error' => '')) {
		$class_details = $this->class_details;
		$class = $class_details->class;
		$class_id = $class_details->id;
		...
		$this->load->view('staff/homework/class_teacher/new_homework', $data);
        $this->staff_footer();
	}


	public function new_homework_action() {
		...
		if ($this->form_validation->run())  {	
			...
			$this->new_homework(); //validation fails, reload page with validation errors
		}
    }
	...
	public function edit_homework($homework_id, $error = array('error' => '')) {
		...
		$data['subjects_option'] = $this->common_model->subjects_option_by_section(school_id, $section_id);
		$data['upload_error'] = $error;
		$this->load->view('staff/homework/class_teacher/edit_homework', $data);
        $this->staff_footer();
	}
    ...
    public function edit_homework_action($homework_id) {
		...
		if ($this->form_validation->run())  {	
			...
			if ( $_FILES['material']['name'] == "" ) { //file is not selected
				...
				redirect(site_url('homework_class_teacher/view_homework/'.$homework_id)); 
	...
	private function restrict_homework($homework_id, $teacher_class_id) {
		...
		$teacher_class = $this->common_model->get_class_details($teacher_class_id)->class;
		if ($homework_class_id == $teacher_class_id) {
			return TRUE;
		} else {
			$this->session->set_flashdata('status_msg_error', "The homework you tried to access was not created for {$teacher_class}");
			redirect(site_url('homework_class_teacher/homework')); //redirect to all homeworks page
		}
	}
