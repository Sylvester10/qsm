...
/* ===== Documentation ===== 
Name: Free_trial_schools_model_ajax
Role: Ajax model for jQuery DataTables
Description: Handles ajax DB processes of school accounts on free trial
...
class Free_trial_schools_model_ajax extends CI_Model {
	...
	function get_records() {
	    ...
	    $where = "activated = 'false' AND mode = 'Free Trial' AND NOT (date_installed < DATE_SUB(CURRENT_TIMESTAMP, INTERVAL {$free_trial_period} DAY) )";	
		$this->db->where($where);
		$query = $this->db->get();
		return $query->result();
	}
	...
	public function modal_extend_free_trial_period($school_id) {
		$y = $this->school_account_model->get_school_info($school_id);
		$date_expiring = get_expiration_date($y->date_installed);
			...
			<div class="modal-dialog">
				<div class="modal-content modal-form-sm">
					...
					. form_open('school_account/extend_free_trial_period/'.$school_id) . 
						...
						'<div class="modal-body">
							<p>Date Expiring: ' . $date_expiring. '</p>
							<div>
