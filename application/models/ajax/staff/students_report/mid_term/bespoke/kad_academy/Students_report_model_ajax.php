...
Description: Handles ajax DB processes of mid-term students reports in a given class in staff's dashboard
Controller: Kad_student_mid_term_reports_[class_teacher, subject_teacher]
Bespoke: KAD Academy
...
class Students_report_model_ajax extends CI_Model {
    ...
    var $table = 'kad_mid_term_student_results';
    ...
    public function actions($result_id) {
        $r = $this->kad_students_mid_term_report_model->get_result_details_by_id($result_id);
        $session = $r->session;
        $term = $r->term;
        $class_id = $r->class_id;
