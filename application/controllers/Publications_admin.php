...
Role: Controller
Description: Publication_management Class controls access to all publications from the admin's end
...
class Publications_admin extends MY_Controller {
	public function __construct() {
		...
		$this->module_restricted_admin(school_id, mod_publication_management); //publication management module
		$this->activation_restricted_admin(school_id);

		//module-level scripts
		$this->admin_module_scripts = array('s_publications'); 
	}
	...
	public function general_announcement() {
		$this->admin_header('General Announcement', 'General Announcement');	
		$data['general_announcement'] = $this->publications_model->get_general_announcement();
		$this->load->view('admin/publications/general_announcement', $data);
        $this->admin_footer();
	}
	...
	public function delete_general_announcement($id) { 
		...
		$this->check_school_data_exists(school_id, $id, 'id', 'general_announcement', 'admin');
	...
	public function staff_announcement() {
		$this->admin_header('Staff Announcement', 'Staff Announcement');	
		$data['staff_announcement'] = $this->publications_model->get_staff_announcement();
		$this->load->view('admin/publications/staff_announcement', $data);
        $this->admin_footer();
	}
	...
	public function delete_staff_announcement($id) { 
		...
		$this->demo_action_restricted_admin();
		//check announcement exists for this school
		$this->check_school_data_exists(school_id, $id, 'id', 'staff_announcement', 'admin');
	...
	public function news() {
		...
		$config["base_url"] = base_url('publications_admin/news');
		...
		$this->load->view('admin/publications/news', $data);
		$this->admin_footer();
	}
	...
	public function single_news($id, $slug) {
		...
		$this->check_school_data_exists(school_id, $id, 'id', 'news', 'admin');
		$this->check_school_data_exists(school_id, $slug, 'slug', 'news', 'admin');
		$news_details = $this->publications_model->get_news_details($id);
		$title = $news_details->title;
		$this->admin_header($title, $title);
		$data['y'] = $news_details;
		$this->load->view('admin/publications/single_news', $data);
		$this->admin_footer();
	}
	...
	public function create_news($error = array('error' => '')) { 
		...
		$this->load->view('admin/publications/create_news', $error);
		$this->admin_footer();
	}
	...
	public function create_news_action() {	
		...
		if ($this->form_validation->run())  {	
			...
			if ( $_FILES['featured_image']['name'] == "" ) { //file is not selected
				...
				redirect(site_url('publications_admin/news'));
				...
				$posted_by = 'Admin';
				$poster_id = $this->admin_details->id;
	...
	public function edit_news($id, $error = array('error' => '')) { 
		...
		$this->load->view('admin/publications/edit_news', $data);
		$this->admin_footer();
	}
	...
	public function edit_news_action($id, $error = array('error' => '')) {	
		...
		if ($this->form_validation->run())  {	
			...
			if ( $_FILES['featured_image']['name'] == "" ) { //file is not selected
				...
				redirect(site_url('publications_admin/edit_news/'.$id)); 
	...
	public function newsletters() {
		...
		$per_page = 5;  //number of items to be displayed per page
        $uri_segment = 3;  //pagination segment id
		$config["base_url"] = base_url('publications_admin/newsletters');
		...
		$this->load->view('admin/publications/newsletters', $data);
		$this->admin_footer();
	}
	...
	public function create_newsletter($error = array('error' => '')) { 
		...
		$this->load->view('admin/publications/create_newsletter', $error);
		$this->admin_footer();
	}
	...
	public function create_newsletter_action() {	
		...
		if ($this->form_validation->run())  {	
			...
			if ( $_FILES['the_file']['name'] == "" ) { //file is not selected
				$this->session->set_flashdata('status_msg_error', 'No file selected.');
				redirect(site_url('publications_admin/newsletters'));
	...
	public function publish_newsletter($id) { 
		...
		$this->check_school_data_exists(school_id, $id, 'id', 'newsletters', 'admin');
	...
	public function term_dates() {
		...
		$this->admin_header('Term Dates', $inner_page_title);
		...
		$this->load->view('admin/publications/term_dates', $data);
        $this->admin_footer();
	}
	
	
	public function term_dates_ajax() {
		$this->load->model('ajax/admin/publications/term_dates_model_ajax', 'current_model');
	...
	public function term_dates_keep_for_student() {
		...
		$per_page = 5;  //number of items to be displayed per page
        $uri_segment = 3;  //pagination segment id
		$config["base_url"] = base_url('publications_admin/term_dates');
	...
	public function update_term_date($id) { 
		...
		$this->check_school_data_exists(school_id, $id, 'id', 'term_dates', 'admin');
		$this->form_validation->set_rules('activity', 'Activity', 'trim|required');
		$this->form_validation->set_rules('term_date', 'Date', 'trim|required');
		$this->form_validation->set_rules('end_date', 'End Date', 'trim');
		if ($this->form_validation->run())  {		
			...
			redirect(site_url('publications_admin/term_dates'));
		} else {
	...
	public function calendar_dates() {
		...
		$this->admin_header('Events Calendar', $inner_page_title);
		...
		$prefs['next_prev_url'] = base_url('publications_admin/calendar_dates'); //url for calendar pagination
		...
		$this->load->view('admin/publications/calendar_dates', $data);
		$this->admin_footer();
	}
	...
	public function calendar_dates_list() {
		...
		$this->admin_header('Events Calendar List', $inner_page_title);
		//config for pagination
        $config = array();
		$per_page = 5;  //number of items to be displayed per page
        $uri_segment = 3;  //pagination segment id: publications_admin/calendar_dates_list/pagination_id
		$config["base_url"] = base_url('publications_admin/calendar_dates_list');
        ...
        $str_links = $this->pagination->create_links();
        $data["links"] = explode('&nbsp;', $str_links); //explode the links 1 2 3 4 into distinct items for styling.
		$this->load->view('admin/publications/calendar_dates_list', $data);
		$this->admin_footer();
	}
	...
	public function edit_calendar_date($id) { 
		...
		$this->check_school_data_exists(school_id, $id, 'id', 'calendar_dates', 'admin');
		$this->form_validation->set_rules('calendar_date', 'Date', 'required');
		$this->form_validation->set_rules('caption', 'Event Caption', 'trim|required');
		$this->form_validation->set_rules('description', 'Description', 'trim|required');
		if ($this->form_validation->run())  {		
	...
	public function bulk_actions_calendar_dates() { 
		...
		redirect(site_url('publications_admin/calendar_dates_list'));
	}
