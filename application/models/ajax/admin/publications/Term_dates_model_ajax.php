...
Description: Handles ajax DB processes of all term dates in admin dashboard
Controller: publications_admin
...
class Term_dates_model_ajax extends CI_Model {
	...
	public function publish_actions($id) {
		$y = $this->publications_model->get_term_date_details($id);
		if ($y->published == 'true') {
			return '<p><a type="button" href="' .base_url('publications_admin/unpublish_term_date/'.$id). '" class="btn btn-default btn-sm btn-block action-btn clickable"> <i class="fa fa-eye-slash" style="color: green"></i> &nbsp; Unpublish </a></p>';
		} else {
			return '<p><a type="button" href="' .base_url('publications_admin/publish_term_date/'.$id). '" class="btn btn-default btn-sm btn-block action-btn clickable"> <i class="fa fa-eye" style="color: green"></i> &nbsp; Publish </a></p>';
		}
	}
	...
	public function term_date_edit_form($id) {
		$y = $this->publications_model->get_term_date_details($id);
		return form_open('publications_admin/update_term_date/'. $id) .
				'<div class="m-b-10">
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-footer">
						<a class="btn btn-sm btn-danger" role="button" href="' .base_url('publications_admin/delete_term_date/'.$id). '"> Yes, Delete </a>
