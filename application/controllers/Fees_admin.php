...
Name: Fees_admin
Role: Controller
Description: Fees Class controls access to all fees pages and functions from the admin's end
Model: Fees_model
...
class Fees_admin extends MY_Controller {
	public function __construct() {
		...
		$this->module_restricted_admin(school_id, mod_fee_management); //fee management module
	...
	public function new_fee_category_ajax() {	
   	...
   	public function edit_fee_category($fee_cat_id) {
		...
		$this->check_school_data_exists(school_id, $fee_cat_id, 'id', 'fee_categories', 'admin');	
	...
	public function new_fee_discount_category_ajax() {	
   	...
   	public function edit_fee_discount_category($discount_cat_id) {
    	//check fee details exists for this school
		$this->check_school_data_exists(school_id, $discount_cat_id, 'id', 'fee_discount_categories', 'admin');	
		$this->form_validation->set_rules('category', 'Category', 'required');
		$this->form_validation->set_rules('discount', 'Discount', 'trim|required');
		if ($this->form_validation->run())  {	
			$category = ucwords($this->input->post('category', TRUE)); 
    ...
    public function edit_class_fees($fee_id) {
    	//check fee details exists for this school
		$this->check_school_data_exists(school_id, $fee_id, 'id', 'fee_details', 'admin');	
	...
	public function partial_payment_details($payment_id) {
		...
		$this->check_school_data_exists(school_id, $payment_id, 'id', 'fee_payment', 'admin');
	...
	public function validate_payment($payment_id) { 
		...
		$validated_by = $this->admin_details->id;
		$this->fees_model->validate_payment($payment_id, $validated_by);
		$f = $this->fees_model->get_fee_payment_details_by_id($payment_id);
		$student_name = $this->common_model->get_student_fullname($f->student_id);	
		$this->session->set_flashdata('status_msg', "{$student_name}'s payment validated successfully.");
		redirect($this->agent->referrer());
	}
	...
	public function invalidate_payment($payment_id) { 
		...
		//check admin level = 1
		$this->check_admin_level();
		$this->fees_model->invalidate_payment($payment_id);
		$f = $this->fees_model->get_fee_payment_details_by_id($payment_id);
		$student_name = $this->common_model->get_student_fullname($f->student_id);	
		$this->session->set_flashdata('status_msg', "{$student_name}'s payment invalidated successfully.");
		redirect($this->agent->referrer());
	}


	public function validate_partial_payment($payment_id, $installment) { 
		//check fee exists for this school
		$this->check_school_data_exists(school_id, $payment_id, 'id', 'fee_payment', 'admin');
		//check admin level = 1
		$this->check_admin_level();
		$validated_by = $this->admin_details->id;
		$this->fees_model->validate_partial_payment($payment_id, $installment, $validated_by);
		$f = $this->fees_model->get_fee_payment_details_by_id($payment_id);
		$student_name = $this->common_model->get_student_fullname($f->student_id);	
		$this->session->set_flashdata('status_msg', "{$student_name}'s partial payment validated successfully.");
		redirect($this->agent->referrer());
	}
	...
	public function invalidate_partial_payment($payment_id, $installment) { 
		...
		//check admin level = 1
		$this->check_admin_level();
		$this->fees_model->invalidate_partial_payment($payment_id, $installment);
		$f = $this->fees_model->get_fee_payment_details_by_id($payment_id);
		$student_name = $this->common_model->get_student_fullname($f->student_id);	
		$this->session->set_flashdata('status_msg', "{$student_name}'s partial payment invalidated successfully.");
		redirect($this->agent->referrer());
	}


	public function validate_all_partial_payment($payment_id) { 
		//check fee exists for this school
		$this->check_school_data_exists(school_id, $payment_id, 'id', 'fee_payment', 'admin');
		//check admin level = 1
		$this->check_admin_level();
		$validated_by = $this->admin_details->id;
		$this->fees_model->validate_all_partial_payment($payment_id, $validated_by);
		$f = $this->fees_model->get_fee_payment_details_by_id($payment_id);
		$student_name = $this->common_model->get_student_fullname($f->student_id);	
		$this->session->set_flashdata('status_msg', "{$student_name}'s payments validated successfully.");
		redirect($this->agent->referrer());
	}
	...
	public function invalidate_all_partial_payment($payment_id) { 
		...
		//check admin level = 1
		$this->check_admin_level();
		$this->fees_model->invalidate_all_partial_payment($payment_id);
		$f = $this->fees_model->get_fee_payment_details_by_id($payment_id);
		$student_name = $this->common_model->get_student_fullname($f->student_id);	
		$this->session->set_flashdata('status_msg', "{$student_name}'s payments invalidated successfully.");
		redirect($this->agent->referrer());
	}
	...
	private function check_admin_level() {
		$admin_level = $this->common_model->get_admin_details($this->session->admin_email)->level;
		if ($admin_level == 1) {
			return TRUE;
		} else {
			$this->session->set_flashdata('status_msg_error', 'You do not have privileges to execute the action you attempted.');
			redirect($this->agent->referrer());
		}
	}
