...
/* ===== Documentation ===== 
Name: Graduated_students_model_ajax
Role: Ajax model for jQuery DataTables
Description: Handles ajax DB processes of graduated students in admin's dashboard
...
class Graduated_students_model_ajax extends CI_Model {
	...
	var $order = array('date_graduated' => 'desc'); 
	...
	function get_records() {
		...
		$this->db->where(array('school_id' => school_id, 'graduated' => 'true'));
	    $query = $this->db->get();
		return $query->result();
	}
	...
	public function actions($id) {
		...
		<p><a type="button" href="#" class="btn btn-default btn-sm btn-block action-btn clickable" data-toggle="modal" data-target="#re_admit'.$id.'"> <i class="fa fa-sign-in" style="color: green"></i> &nbsp; Re-admit Student</a></p>
	...
	public function re_admit_student_form($id) {
		$y = $this->common_model->get_student_details_by_id($id);
		return form_open('students_admin/re_admit_graduated_student/'.$id). 
			'<div class="m-b-10">Class</div>
			...
			'</select>
			<div class="m-t-20">
				<button class="btn btn-primary">Re-admit</button>
			</div>'
		.form_close();
	}
	
	
	public function modal_re_admit_student_form($id) {
		$y = $this->common_model->get_student_details_by_id($id);
		return	'<div class="modal fade" id="re_admit'.$id.'" role="dialog">
				<div class="modal-dialog">
					<div class="modal-content modal-form-sm">
						<div class="modal-header">
							...
							<h4 class="modal-title"> <i class="fa fa-sign-in"></i> Re-admit Student: ' . $this->common_model->get_student_fullname($id) . '</h4>
						</div><!--/.modal-header-->
						<div class="modal-body">'
							. $this->re_admit_student_form($id) .
						'</div>
					</div>
				</div>
			</div>';
	...
	public function modals($id) {
		return 	$this->modal_options($id) . 
				$this->modal_re_admit_student_form($id) .
