...
/* ===== Documentation ===== 
Name: All_schools_model_ajax
Role: Ajax model for jQuery DataTables
Description: Handles ajax DB processes of school accounts
...
class All_schools_model_ajax extends CI_Model {
	...
	public function activate_actions($school_id) {
		$y = $this->school_account_model->get_school_info($school_id);
		if ($y->activated == 'false') {
			return 
			...
			return '<p><a type="button" class="btn btn-default btn-sm btn-block action-btn clickable" href="' . base_url('school_account/deactivate_school_account/'.$school_id) .'"> <i class="fa fa-times" style="color: red"></i> &nbsp; De-activate Account </a></p>';
		}
	}
	...
	public function actions($school_id) {
		...
		$this->activate_actions($school_id) . 
