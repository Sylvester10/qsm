...
/* ===== Documentation ===== 
Name: School_publications_admin
Role: Controller
Description: School_publications_admin Class controls access to all publications from the admin's end
...
class School_publications_admin extends MY_Controller {
	...
	public function news() {
		...
		$this->load->view('admin/school_publications/news', $data);
		$this->admin_footer();
	}
	...
	public function single_news($id, $slug) {
		...
		$data['y'] = $news_details;
		$this->load->view('admin/school_publications/single_news', $data);
		$this->admin_footer();
	}
	...
	public function newsletters() {
		$this->admin_header('Newsletters', 'Newsletters');	
		//config for pagination
        $config = array();
		$per_page = 5;  //number of items to be displayed per page
        $uri_segment = 3;  //pagination segment id
		$config["base_url"] = base_url('school_publications_admin/newsletters');
		...
		$this->load->view('admin/school_publications/newsletters', $data);
		$this->admin_footer();
	}
	...
	public function term_dates() {
		$this->admin_header('Term Dates', 'Term Dates');
		//config for pagination
        $config = array();
		$per_page = 5;  //number of items to be displayed per page
        $uri_segment = 3;  //pagination segment id
		$config["base_url"] = base_url('school_publications_admin/term_dates');
		...
		$this->load->view('admin/school_publications/term_dates', $data);
		$this->admin_footer();
	}
	...
	public function calendar_dates() {
		$this->admin_header('Events Calendar', 'Events Calendar');
		...
		$prefs['next_prev_url'] = base_url('school_publications_admin/calendar_dates'); //url for calendar pagination
		...
		$this->load->view('admin/school_publications/calendar_dates', $data);
		$this->admin_footer();
	}
	...
	public function calendar_dates_list() {
		$this->admin_header('Events Calendar List', 'Events Calendar List');
		//config for pagination
        $config = array();
		$per_page = 5;  //number of items to be displayed per page
        $uri_segment = 3;  //pagination segment id: school_publications_admin/calendar_dates_list/pagination_id
		$config["base_url"] = base_url('school_publications_admin/calendar_dates_list');
        ...
        $str_links = $this->pagination->create_links();
        $data["links"] = explode('&nbsp;', $str_links); //explode the links 1 2 3 4 into distinct items for styling.
		$this->load->view('admin/school_publications/calendar_dates_list', $data);
		$this->admin_footer();
	}
