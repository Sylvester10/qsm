...
/* ===== Documentation ===== 
Name: School_library_staff
Role: Controller
Description: School_library_staff Class controls access to all library books pages and functions from the staff's end
...
class School_library_staff extends MY_Controller {
	public function __construct() {
		parent::__construct();
		$this->staff_restricted(); //allow only logged in users to access this class
		$this->load->model('school_library_model');
		$this->staff_role_restricted('Librarian'); //only staff with this role can access this module
		...
		$this->module_restricted_staff(school_id, mod_school_library); //school library module
		$this->activation_restricted_staff(school_id); 

		//module-level scripts
		$this->staff_module_scripts = array('s_school_library');
	}
	...
	public function index() {
		...
		$this->staff_header('All Books', $inner_page_title);	
		...
		$this->load->view('staff/school_library/all_books', $data);
        $this->staff_footer();
	}
	
	
	public function all_books_ajax() {
		$this->load->model('ajax/staff/school_library/all_books_model_ajax', 'current_model');
	...
	public function new_book() {
		$this->staff_header('New Book', 'New Book');	
		$this->load->view('staff/school_library/new_book');
        $this->staff_footer();
	}
	...
	public function edit_book($book_id) {
		...
		$this->check_school_data_exists(school_id, $book_id, 'id', 'school_library', 'staff');	
		...
		$data['y'] = $book_details;
		$this->load->view('staff/school_library/edit_book', $data);
        $this->staff_footer();
	}
	...
	public function borrowed_books() {
		...
		$this->staff_header('Borrowed Books', $inner_page_title);	
		$total_unique_books = count($this->school_library_model->get_all_books());
		$total_books_copies = $this->school_library_model->get_books_copies();
		$total_available_copies = $this->school_library_model->get_available_books_copies();
		$data['total_unique_books'] = $total_unique_books;
		...
		$data['due_books'] = $this->school_library_model->get_due_books();
		$data['total_due_books'] = $this->school_library_model->get_total_due_books();
		$this->load->view('staff/school_library/borrowed_books', $data);
        $this->staff_footer();
	}
	
	
	public function borrowed_books_ajax() {
		$this->load->model('ajax/staff/school_library/all_borrowed_books_model_ajax', 'current_model');
		$list = $this->current_model->get_records();
		$data = array();
		foreach ($list as $y) {
			$book_id = $y->book_id;
	...
	public function return_book($bb_id) { 
		//check book exists for this school
		$this->check_school_data_exists(school_id, $bb_id, 'id', 'borrowed_books', 'staff');	
