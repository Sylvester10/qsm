jQuery(document).ready(function ($) {
 "use strict";
	...
	$('#new_book_form').submit(function(e) {
		...
		$.ajax({
			url: base_url + 'school_library_admin/add_new_book_ajax', 
			type: 'POST',
			data: form_data, 
	...
	$('#edit_book_form').submit(function(e) {
		...
		var redirect_url = base_url + 'school_library_admin';
		$.ajax({
			url: base_url + 'school_library_admin/edit_book_ajax/' + book_id, 
			type: 'POST',
			data: form_data, 
			success: function(msg) {
				if (msg == 1) {
	...
	var all_books_table = $('#all_books_table').DataTable({ 
		...
		ajax: {
			url: base_url + 'school_library_admin/all_books_ajax',
			dataType: "json",
			type: "POST",
	...
	var borrowed_books_table = $('#borrowed_books_table').DataTable({ 
		...
		ajax: {
			url: base_url + 'school_library_admin/borrowed_books_ajax',
			dataType: "json",
			type: "POST",
			data: { 'q2r_secure' : csrf_hash } //cross site request forgery protection
		},
