...
/* ===== Documentation ===== 
Name: School_publications_parent
Role: Controller
Description: School_publications_parent Class controls access to all publications from the parent's end
...
class School_publications_parent extends MY_Controller {
	public function __construct() {
		...
		$this->login_module_restricted(school_id, mod_parent_login, mod_publication_management, 'parent'); 
	...
	public function news() {
		...
		$this->load->view('parent/school_publications/news', $data);
		$this->parent_footer();
	}
	...
	public function single_news($id, $slug) {
		...
		$this->check_school_data_exists(school_id, $id, 'id', 'news', 'parent');
		$this->check_school_data_exists(school_id, $slug, 'slug', 'news', 'parent');
		$news_details = $this->publications_model->get_news_details($id);
		$title = $news_details->title;
		$this->parent_header($title, $title);
		$data['y'] = $news_details;
		$this->load->view('parent/school_publications/single_news', $data);
		$this->parent_footer();
	}
	...
	public function newsletters() {
		$this->parent_header('Newsletters', 'Newsletters');	
		//config for pagination
        $config = array();
		$per_page = 5;  //number of items to be displayed per page
        $uri_segment = 3;  //pagination segment id
		$config["base_url"] = base_url('school_publications_parent/newsletters');
		...
		$this->load->view('parent/school_publications/newsletters', $data);
		$this->parent_footer();
	}
	...
	public function term_dates() {
		$this->parent_header('Term Dates', 'Term Dates');
		//config for pagination
        $config = array();
		$per_page = 5;  //number of items to be displayed per page
        $uri_segment = 3;  //pagination segment id
		$config["base_url"] = base_url('school_publications_parent/term_dates');
		...
		$this->load->view('parent/school_publications/term_dates', $data);
		$this->parent_footer();
	}
	...
	public function calendar_dates() {
		$this->parent_header('Events Calendar', 'Events Calendar');
		...
		$prefs['next_prev_url'] = base_url('school_publications_parent/calendar_dates'); //url for calendar pagination
		...
		$this->load->view('parent/school_publications/calendar_dates', $data);
		$this->parent_footer();
	}
	...
	public function calendar_dates_list() {
		$this->parent_header('Events Calendar List', 'Events Calendar List');
		//config for pagination
        $config = array();
		$per_page = 5;  //number of items to be displayed per page
        $uri_segment = 3;  //pagination segment id: school_publications_parent/calendar_dates_list/pagination_id
		$config["base_url"] = base_url('school_publications_parent/calendar_dates_list');
        ...
        $str_links = $this->pagination->create_links();
        $data["links"] = explode('&nbsp;', $str_links); //explode the links 1 2 3 4 into distinct items for styling.
		$this->load->view('parent/school_publications/calendar_dates_list', $data);
		$this->parent_footer();
	}
