...
/* ===== Documentation ===== 
Name: Expired_annual_sub_schools_model_ajax
Role: Ajax model for jQuery DataTables
Description: Handles ajax DB processes of school accounts on annual subscription whose account have expired
...
class Expired_annual_sub_schools_model_ajax extends CI_Model {
	...
	public function actions($school_id) {
		...
		<p><a type="button" href="#" class="btn btn-default btn-sm btn-block action-btn clickable" data-toggle="modal" data-target="#extend'.$school_id.'"> <i class="fa fa-clock-o" style="color: green"></i> &nbsp; Extend Subscription Period </a></p>
	...
	public function modal_extend_subscription_period($school_id) {
		$y = $this->school_account_model->get_school_info($school_id);
		$date_expired = get_renewal_date($y->activated, $y->date_activated);
			...
			<div class="modal-dialog">
				<div class="modal-content modal-form-sm">
					<div class="modal-header">
						...
						<h4 class="modal-title">Extend Subscription Period: ' . $y->school_name . '</h4>
					</div><!--/.modal-header-->'
					
					. form_open('school_account/extend_subscription_period/'.$school_id) . 
	...
	public function modals($school_id) {
		return 	$this->modal_options($school_id) . 
				...
				$this->modal_extend_subscription_period($school_id) .
