...
class All_students_model_ajax extends CI_Model {
	...
	function get_records() {
		...
		$this->db->where(array('school_id' => school_id, 'graduated' => 'false'));
	    $query = $this->db->get();
		return $query->result();
	}
	...
	public function revoke_status($id) {
		$y = $this->common_model->get_student_details_by_id($id);
		if ($y->revoked == 'false') {
			return '<b class="text-success">No</b>';
		} else {
	...
	public function suspension_status_actions($id) {
		$y = $this->common_model->get_student_details_by_id($id);
		if ($y->suspended == 'false') {
			return '<p><a type="button" href="#" class="btn btn-default btn-sm btn-block action-btn clickable" data-toggle="modal" data-target="#suspend'.$id.'"> <i class="fa fa-ban" style="color: red"></i> &nbsp; Suspend Student </a></p>';
		} else {
	...
	public function revoke_status_actions($id) {
		$y = $this->common_model->get_student_details_by_id($id);
		if ($y->revoked == 'false') {
			return '<p><a type="button" href="#" class="btn btn-default btn-sm btn-block action-btn clickable" data-toggle="modal" data-target="#revoke'.$id.'"> <i class="fa fa-times" style="color: red"></i> &nbsp; Revoke Admission </a></p>';
		} else {
			return '<p><a type="button" class="btn btn-default btn-sm btn-block action-btn clickable" href="' . base_url('students_admin/unrevoke_student_admission/'.$id) .'"> <i class="fa fa-check" style="color: green"></i> &nbsp; Unrevoke Admission </a></p>';
		}
	}
	...
	public function actions($id) {
		...
		. $this->suspension_status_actions($id)

		. $this->revoke_status_actions($id) .
	...
	public function suspension_duration_form($id) {
		return form_open('students_admin/suspend_student/'.$id). 
			...
			<div class="m-t-10">
				<label>Suspension Duration (e.g. 1 week)</label> <br />
	...
	public function modal_suspend_student($id) {
		$y = $this->common_model->get_student_details_by_id($id);
		return	'<div class="modal fade" id="suspend'.$id.'" role="dialog">
				<div class="modal-dialog">
