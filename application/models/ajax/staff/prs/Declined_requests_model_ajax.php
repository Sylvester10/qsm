...
/* ===== Documentation ===== 
Name: Declined_requests_model_ajax
Role: Ajax model for jQuery DataTables
Description: Handles ajax DB processes of all declined requests raised by staff 
...
class Declined_requests_model_ajax extends CI_Model {
	...
	function get_records($session, $term) {
		...
		$this->db->where(array('school_id' => school_id, 'session' => $session, 'term' => $term, 'initiator_id' => $this->staff_details->id, 'raised_by' => 'Staff', 'status' => 'declined'));
	    $query = $this->db->get();
		return $query->result();
	}
