...
Description: Handles ajax DB processes of all borrowed books in staff's dashboard
...
class All_borrowed_books_model_ajax extends CI_Model {
	...
	public function actions($bb_id) {
		$y = $this->school_library_model->get_borrowed_book_details($bb_id);
		return '<p><a type="button" class="btn btn-default btn-sm btn-block action-btn clickable" href="' . base_url('school_library_staff/edit_book/'.$y->book_id) .'"> <i class="fa fa-pencil" style="color: green"></i> &nbsp; Edit Book </a></p>
	...
	public function return_book_partial_copies_form($bb_id) {
		...
		return form_open('school_library_staff/return_book_partial_copies/'.$bb_id).
			'<div class="m-b-10">
	...
	public function message_staff_form($bb_id) {
		return form_open('school_library_staff/message_staff/'.$bb_id). 
			'<div>
