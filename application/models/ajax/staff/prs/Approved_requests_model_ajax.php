...
Description: Handles ajax DB processes of all approved requests raised by staff 
...
class Approved_requests_model_ajax extends CI_Model {
	...
	function get_records($session, $term) {
		...
		$this->db->where(array('school_id' => school_id, 'session' => $session, 'term' => $term, 'initiator_id' => $this->staff_details->id, 'raised_by' => 'Staff', 'status' => 'approved'));
	    $query = $this->db->get();
		return $query->result();
	}
