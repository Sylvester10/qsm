...
if ($total_jobs > 0) {
	...
	$url = 'cron_jobs/clear_cron_jobs_db/'.$cron_name;
	$title = 'Clear Cron Jobs: ' . $cron_title . ' (' . $total_jobs . ')';
	echo modal_confirm_action('clear_jobs_db', $title, $custom_msg, $url);
}
...
<div class="new-item">
	...
	<?php if ($total_jobs > 0) { ?>
		<button class="btn btn-default btn-sm button-adjust" data-toggle="modal" data-target="#clear_jobs_db" title="Clear all jobs for this daemon"><i class="fa fa-trash"></i> Clear Jobs</button>
	<?php } ?>
...
echo modal_bulk_actions('cron_jobs/bulk_actions_cron_jobs_db', $options_array); ?>

	<div class="table-scroll">
		<table id="cron_jobs_db_table" class="table table-bordered table-hover cell-text-middle" style="text-align: left">
