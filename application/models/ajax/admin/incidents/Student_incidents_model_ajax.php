...
/* ===== Documentation ===== 
Name: Student_incidents_model_ajax
Role: Ajax model for jQuery DataTables
Description: Handles ajax DB processes of student incidents in admin's dashboard
...
class Student_incidents_model_ajax extends CI_Model {
	...
	function get_records($student_id) {
		$this->the_query();
		if($_POST['length'] != -1)
	...
	function count_filtered_records($student_id) {
		$this->the_query();
		$this->db->where('student_id', $student_id);
		$query = $this->db->get();
	...
	public function count_all_records($student_id) {
		$this->db->where('student_id', $student_id);
		$this->db->from($this->table);
