...
class Cron_jobs_db_model_ajax extends CI_Model {
	public function __construct() {
		parent::__construct();
		$this->load->model('cron_jobs_model');
	}
	
	var $table = 'cron_jobs_db';
	var $column_order = array(null, 'id', 'name', 'title', 'status', 'time'); //set column field database for datatable orderable
	var $column_search = array('id', 'name', 'title', 'status', 'time'); //set column field database for datatable searchable 
	...
	public function modals($cj_id) {
		$y = $this->cron_jobs_model->get_cron_job_db_details($cj_id);
		$modal_delete_confirm = modal_delete_confirm($cj_id, $y->title, 'cron job', 'cron_jobs/delete_cron_job_db');
