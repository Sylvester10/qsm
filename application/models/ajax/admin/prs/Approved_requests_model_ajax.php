...
/* ===== Documentation ===== 
Name: Approved_requests_model_ajax
Role: Ajax model for jQuery DataTables
Description: Handles ajax DB processes of all approved requests
...
class Approved_requests_model_ajax extends CI_Model {
	...
	function get_records($session, $term) {
		...
		$this->db->where(array('school_id' => school_id, 'session' => $session, 'term' => $term, 'status' => 'approved'));
		$query = $this->db->get();
		return $query->result();
	}
	...
	public function request_status_actions($id) {
		return '<p><a class="btn btn-default btn-sm btn-block action-btn" href="' .base_url('prs_admin/unapprove_request/'.$id). '"> <i class="fa fa-times" style="color: red"></i> &nbsp; Unapprove </a></p>
